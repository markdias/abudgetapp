...
final class AppState: ObservableObject {
            ...
            .sink(receiveCompletion: { [weak self] completion in
                if let strongSelf = self {
                    ...
                    if case .failure(let err) = completion {
                        strongSelf.error = "Failed to fetch accounts: \(err.localizedDescription)"
                ...
                if let strongSelf = self {
                    ...
                    strongSelf.updateTransactionsFromAccounts()
                    strongSelf.updateBudgetItemsFromAccounts()
                }
            })
    ...
    // MARK: - Transfer Schedule Methods
    func addTransferSchedule(_ submission: TransferScheduleSubmission) async throws -> Bool {
        do {
            let schedule = try await APIService.shared.addTransferSchedule(transfer: submission)
            await MainActor.run {
                self.transferSchedules.append(schedule)
            }
            return true
        } catch {
            await MainActor.run {
